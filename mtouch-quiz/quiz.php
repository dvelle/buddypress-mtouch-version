  ...
  <table class="widefat">
    <thead>
      <tr>
        ...
        <th scope="col"><?php _e('Reward Badge', 'mtouchquiz') ?></th>
...
$all_quiz = $wpdb->get_results("SELECT Q.ID,Q.name,Q.added_on,(SELECT COUNT(*) FROM {$wpdb->prefix}mtouchquiz_question WHERE quiz_id=Q.ID) AS question_count, Q.badge_url
...
if (count($all_quiz)) {
    ...
    <th scope="row" style="text-align: center;"><?php echo $quiz->ID ?></th>
      ...
      <?php
      if($quiz->badge_url){?>
      <td><img src="<?php echo $quiz->badge_url; ?>" /></td>
       
      <?php } else { ?>
      <td>No Badge has been assigned</td> 
      <?php
      }
      ?>
