<?php
	...
	if (true) {
		...
		$quiz_options = $wpdb->get_row($wpdb->prepare("SELECT name,description,answer_mode,single_page,show_hints,show_start,show_final,multiple_chances,final_screen,random_questions,random_answers,badge_url,assigned_module FROM {$wpdb->prefix}mtouchquiz_quiz WHERE ID=%d", $quiz_id));			 		
		...
		$badge = stripslashes($quiz_options->badge_url);
		$mod_id = stripslashes($quiz_options->assigned_module);
...
<div id="mtq_quiz_results_bubble-<?php echo $mtqid ?>" class="mtq_quiz_results_bubble"> <div id="mtq_quiz_results-<?php echo $mtqid ?>" class="mtq_quiz_results"><?php echo str_replace('%%QUIZ_NAME%%','<em>'.stripslashes($quiz_options->name).'</em>',$final_screen);?><br>    
  ...
  <div id="theBadgeAward" style="display:none;"><?php _e('You earned a new badge.', 'mtouchquiz'); ?> <img src="<?php echo $badge; ?>" /></div><?php if  ( $mtq_form_present && ! ( $inform ) ) { ?>
...
?>
<div id="modURL"style="display:none;"><?php echo $mod_id; ?></div>
