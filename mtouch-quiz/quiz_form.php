...
$dquiz = array();
if($action == 'edit') {
	$dquiz = $wpdb->get_row($wpdb->prepare("SELECT name,description,final_screen,answer_mode,single_page,show_hints,show_start, show_final,multiple_chances,random_questions,random_answers,badge_url,assigned_module FROM {$wpdb->prefix}mtouchquiz_quiz WHERE ID=%d", $_REQUEST['quiz']));
	...
	$badge = stripslashes($dquiz->badge_url);
	$assigned_module = stripslashes($dquiz->assigned_module);
...
?>
  <form name="post" action="<?php echo $GLOBALS['wpframe_plugin_folder'] ?>/quiz_action.php" method="post" id="post" enctype="multipart/form-data">
    <div id="poststuff">
	...
	<?php  } ?>
		...
		<div class="postbox">
			<h3 class="hndle"> <span>
				<?php _e('Achievement Badge', 'mtouchquiz') ?>
				</span> </h3>
			<div class="inside">
				<?php if($badge != null){ ?>
					Current Badge : <img src="<?php echo $badge;?>" ><br/> <br/>
					<?php } else {?>
					<b>No Badge Currently Assigned.	</b><br/> <br/>				
					<?php } ?>
					Upload Badge Image: <input type="file" name="file" id="file"><br/>
			</div>
		</div>
	...
	<?php  } ?>
	  ...
	  function Module_menu($parent=0)
		{
		global $post,$wpdb;
		
			$all_modules = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}posts WHERE post_type='things_module'");
			//print_r($all_modules); 
			if(count($all_modules)) 
			{
				foreach($all_modules as $m)
				{
					...
					<li>						
						<ul>
							<?php
							if($m->post_title=="Auto Draft"){
								//Do Nothing
							} else {
								if($m->ID == $assigned_module)
								{
									?> 
									<input type="radio" value="<?php echo $m->ID;?>" name="module" checked="checked"><?php echo $m->post_title; ?></input>
									<?php
								} else {
									?> 
									<input type="radio" value="<?php echo $m->ID;?>" name="module" ><?php echo $m->post_title; ?></input>								
									<?php
								}
							}
							?>
				...
				?>
				<li colspan="4"><?php _e('No Modules have been created.', 'mtouchquiz') ?></li>				
			<?php
			}
		}
	  ?>
	<div class="postbox">
        <h3 class="hndle"> <span>
			<?php _e('Select Educational Module to Associate Quiz With', 'mtouchquiz') ?>
			</span> </h3>
        <div class="inside">
		<ul>
		<?php					
